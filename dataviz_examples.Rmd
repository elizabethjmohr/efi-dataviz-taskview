...
# Get forecasts
forecasts <- read_csv("https://data.ecoforecast.org/analysis/combined_forecasts_scores.csv.gz")
```
...
ggplot(forecasts %>% 
  filter(theme == "aquatics", target == "temperature", !team == "EFInull"), aes(x = crps)) +
  geom_bar()+
  scale_x_binned()+
  theme_minimal_hgrid()+
...
ggplot(tempSample, aes(x = temp, fill = stat(x > 19))) + 
